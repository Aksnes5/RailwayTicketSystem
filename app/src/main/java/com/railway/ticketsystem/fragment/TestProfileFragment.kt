...
class TestProfileFragment : Fragment() {
    ...
    private fun initLaunchers() {
        try {
            galleryLauncher = registerForActivityResult(ActivityResultContracts.GetContent()) { uri ->
                try {
                    uri?.let { 
                        Toast.makeText(requireContext(), "选择了图片: $uri", Toast.LENGTH_SHORT).show()
                        // 暂时不保存，只显示提示
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
            ...
            cameraLauncher = registerForActivityResult(ActivityResultContracts.TakePicturePreview()) { bitmap ->
                try {
                    bitmap?.let { 
                        Toast.makeText(requireContext(), "拍摄了照片", Toast.LENGTH_SHORT).show()
                        // 暂时不保存，只显示提示
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
